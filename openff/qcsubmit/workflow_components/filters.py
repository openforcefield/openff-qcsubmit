    ...
    def _apply(self, molecules: list[Molecule], toolkit_registry: ToolkitRegistry) -> ComponentResult:
        ...
        for toolkit in toolkit_registry.registered_toolkits:
            ...
            raise ModuleNotFoundError(
                "Either openeye or rdkit must be registered with the toolkit registry to use" "the weight filter.",
            )
...
class ElementFilter(ToolkitValidator, CustomWorkflowComponent):
    ...
    @classmethod
    def description(cls) -> str:
        return "Filter out molecules who contain elements not in the allowed element list."
    ...
    def _apply_init(self, result: ComponentResult) -> None:
        self._cache["elements"]: list[str | int] = [
...
class CoverageFilter(ToolkitValidator, CustomWorkflowComponent):
    ...
    allowed_ids: set[str] | None = Field(
        None,
        description=(
            "The SMIRKS parameter ids of the parameters which are allowed to be exercised by the molecules. "
            "Molecules should use at least one of these ids to be passed by the component.",
        ),
    )
    filtered_ids: set[str] | None = Field(
        None,
        description=(
...
class RotorFilter(ToolkitValidator, CustomWorkflowComponent):
    ...
    maximum_rotors: int | None = Field(
        4,
        description=(
            "The maximum number of rotatable bonds allowed in the molecule, if `None` the molecule has no maximum "
            "limit on rotatable bonds.",
        ),
    )
    minimum_rotors: int | None = Field(
        None,
        description=(
            "The minimum number of rotatble bonds allowed in the molecule, if `None` the molecule has no limit to the "
            "minimum number of rotatble bonds.",
        ),
    )
    ...
    def _apply_init(self, result: ComponentResult) -> None:
        ...
        if self.maximum_rotors and self.minimum_rotors:
            if self.maximum_rotors < self.minimum_rotors:
                raise ValueError("The maximum number of rotors should >= the minimum to ensure some molecules pass.")
    ...
    def _apply(self, molecules: list[Molecule], toolkit_registry: ToolkitRegistry) -> ComponentResult:
        ...
        for molecule in molecules:
            ...
            rotatable_bonds = molecule.find_rotatable_bonds(toolkit_registry=toolkit_registry)
...
class SmartsFilter(ToolkitValidator, CustomWorkflowComponent):
    ...
    allowed_substructures: list[str] | None = Field(
        None,
        description="The list of allowed substructures which should be tagged with indices.",
    )
    filtered_substructures: list[str] | None = Field(
        None,
    ...
    def _validate_mutually_exclusive(cls, values):
        ...
        assert allowed_substructures is not None or filtered_substructures is not None, message
        ...
        for molecule in molecules:
            if self.allowed_substructures is not None:
                for substructure in self.allowed_substructures:
                    if molecule.chemical_environment_matches(query=substructure, toolkit_registry=toolkit_registry):
                        result.add_molecule(molecule=molecule)
                        break
                else:
...
class RMSDCutoffConformerFilter(ToolkitValidator, CustomWorkflowComponent):
    ...
    def _prune_conformers(self, molecule: Molecule) -> None:
        ...
        if no_conformers > 1 and self.cutoff >= 0.0:
            ...
            confs = [molecule.conformers[j] for j, add_bool in enumerate(uniq) if add_bool]
...
class ScanFilter(ToolkitValidator, CustomWorkflowComponent):
    ...
    scans_to_include: list[str] | None = Field(
        None,
    ...
    scans_to_exclude: list[str] | None = Field(
        None,
    ...
    _check_smarts = validator("scans_to_include", "scans_to_exclude", each_item=True, allow_reuse=True)(
        check_environments,
    )
    ...
    def _validate_mutally_exclusive(cls, values):
        ...
        message = "exactly one of `scans_to_include` and `scans_to_exclude` must be specified"
    ...
    def _apply(self, molecules: list[Molecule], toolkit_registry: ToolkitRegistry) -> ComponentResult:
        ...
        for molecule in molecules:
            ...
            for env in target_environments:
                ...
                matches = molecule.chemical_environment_matches(query=env, toolkit_registry=toolkit_registry)
...
class ChargeFilter(ToolkitValidator, CustomWorkflowComponent):
    """
    Filter molecules if their formal charge is not in the `charges_to_include` list or is in the `charges_to_exclude`
    list.
    """
    ...
    def _apply(self, molecules: list[Molecule], toolkit_registry: ToolkitRegistry) -> ComponentResult:
        ...
        for molecule in molecules:
            ...
            if (self.charges_to_include is not None and total_charge not in self.charges_to_include) or (
                self.charges_to_exclude is not None and total_charge in self.charges_to_exclude
            ):
                result.filter_molecule(molecule=molecule)
